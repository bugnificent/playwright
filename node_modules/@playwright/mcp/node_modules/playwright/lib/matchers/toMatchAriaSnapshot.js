...
async function toMatchAriaSnapshot(receiver, expectedParam, options = {}) {
  ...
  if ((0, import_utils.isString)(expectedParam)) {
    ...
    if (expectedParam?.name) {
      ...
      expectedPath = testInfo._resolveSnapshotPath("aria", [(0, import_util.trimLongString)(fullTitleWithoutSpec) + ".aria.yml"], true);
      if (!await (0, import_util.fileExistsAsync)(expectedPath)) {
        const legacyPath = testInfo._resolveSnapshotPath("aria", [(0, import_util.trimLongString)(fullTitleWithoutSpec) + ".yml"], true);
